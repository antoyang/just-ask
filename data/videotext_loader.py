...
class VideoText_Dataset(Dataset):
    ...
    def _bert_tokenizer(self, text):
        tokens = th.tensor(self.bert_tokenizer.encode(text, add_special_tokens=True, padding='max_length',
                                                      max_length=self.max_words, truncation=True), dtype=th.long)
        return tokens

    def __getitem__(self, idx):
        text = self.data['sentence'].values[idx]
        text_embd = th.tensor(self.bert_tokenizer.encode(text,
        ...
        if len(video) < self.max_feats:
            ...
            if len(video) < self.max_feats:  # pad
                video = th.cat([video, th.zeros(self.max_feats - len(video), video.shape[1])])
        else:
            sampled = []
